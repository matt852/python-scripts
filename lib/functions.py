...
# Returns True if provided host is wireless controller in the wireless controller subnet
def wifiCheck(host):
	# Check if host is a wireless controller by IP address
	if "10.x.x." in host:
		# If so, return true and last digit of wireless controller
		ip = list(host)
		print "Debug check"
		return (True, ip[-1])
	else:
...
# Example: find the IP address (searchTerm) after interface GigabitEthernet0/1 (startTerm)
...
def readFromFile(inputFileName):
	# Open file provided by user to import content from
